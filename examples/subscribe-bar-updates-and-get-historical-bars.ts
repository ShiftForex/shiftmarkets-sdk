...
async function onAppReady() {
  ...
  const chart = new Chart(ws, exchange, instrument);

  const startTime = new Date(Date.now() - 500 * 3_600_000);
  const endTime = new Date();

  // Get historical bars for the last 500 hours with hour period
  const historicalBars = await chart.getHistoricalBars(
    startTime,
    endTime,
    "hour"
  );
